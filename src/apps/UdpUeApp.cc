...
void UdpUeApp::updateDestAddresses() {
    ...
    // 你可以根据实际情况修改路径
    MobiEdgeSim::Orchestrator *orch =
            check_and_cast<MobiEdgeSim::Orchestrator*>(
                    getSimulation()->getSystemModule()->getSubmodule(
                            "orchestrator"));

    if (!orch) {
        EV_WARN
                       << "Orchestrator module not found, cannot update destination addresses.\n";
        return;
    }
    ...
    if (mobilityModule) {
        auto mobility = check_and_cast<veins::VeinsInetMobility*>(
                mobilityModule);
    ...
    EV << "current latitude" << currentCoord.x << "longitude" << currentCoord.y
              << endl;
...
void UdpUeApp::processPacket(inet::Packet *packet) {
    EV << "UdpUeApp::processPacket received packet: " << packet->getName()
              << endl;
    ...
    if (srcAddrTag) {
        ...
        EV << "Saved RTT to rttMap[" << hostName << "] = " << delay << "s"
                  << endl;
    }
    // 父类处理
...
void UdpUeApp::sendPacket() {
    for (auto &addr : destAddresses) {
        // 每个地址都要做一次新建packet，以免多次sendTo同一个对象
